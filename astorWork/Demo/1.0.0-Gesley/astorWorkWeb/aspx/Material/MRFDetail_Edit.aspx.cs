    ...
    public partial class MRFDetail_Edit : System.Web.UI.Page
    {
        ...
        protected void uxMRFDetail_ItemDataBound(object sender, GridItemEventArgs e)
        {
            if (e.Item is GridDataItem)
            {
                ...
                if (item.GetDataKeyValue("Status").ToString() != "Requested")
                {
            ...
            if (Page.IsValid )
            {
                try
                {
                    if (uxMRFNo.Text == "")
                    {
                        ...
                        {
                            api.SaveMRFMaster(objMRFModel);
                            Master.DisplayAlertMessage(objMRFMaster.MRFNo + "MRF saved successfully.");
        ...
        protected void uxRadWizard_FinishButtonClick(object sender, WizardEventArgs e)
        {
            ...
            closeurl = closeurl.Replace("/astorWorkWeb/aspx", "..");
        ...
        {
            var MRFNo = MRFMasters.Where(w => w.Block == uxBlock.SelectedValue && w.Level == uxLevel.SelectedValue && w.Zone==uxZone.SelectedValue && w.MRFNo != uxMRFNo.Text).FirstOrDefault();
            if (MRFNo != null)
            {
