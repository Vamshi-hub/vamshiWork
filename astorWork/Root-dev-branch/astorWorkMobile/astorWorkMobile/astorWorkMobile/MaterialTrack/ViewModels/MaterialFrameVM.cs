    ...
    public class MaterialFrameVM : MasterVM
    {
        ...
        public bool ShowUnknownTag
        {
            get
            {
                try
                {
                    return Tracker != null && Tracker.Count > 0 && string.IsNullOrEmpty(Tracker.FirstOrDefault().label);
                }
                catch (Exception ex)
                {
        ...
        public Tracker QRTracker
        {
            get
            {
                try
                {
                    return Tracker.Where(t => t.type == "QR Code").FirstOrDefault();
                }
                catch (Exception ex)
                {
                    throw ex;
        ...
        public Tracker RFIDTracker
        {
            get
            {
                try
                {
                    ...
                    return Tracker.Where(t => t.type == "RFID").FirstOrDefault();
                }
                catch (Exception ex)
                {
        ...
        public bool ShowNonAssociateAlert => Scanning && Tracker == null && Tracker.Count < 1;
        ...
        private void AttchButtonClicked()
        {
            try
            {
                ...
                if (Scanning)
                {
                    if (Tracker == null || Tracker.Count == 0 || Material == null)
                    {
                    ...
                    else
                    {
                            ...
                            .ContinueWith((t) =>
                        {
                            ...
                            else
                            {
                                ...
                                //Navigation.PopAsync();
                                DisplaySnackBar($"{Tracker?.FirstOrDefault()?.label} has been associated with {Material.markingNo}", Enums.PageActions.PopAsync, Enums.MessageActions.Success, null, null);
                            }
                        }, TaskScheduler.FromCurrentSynchronizationContext());
                    }
                }
