...
namespace astorWorkMaterialTracking.Controllers
{
    ...
    public class NotificationTimerController : CommonNotificationTimerController
    {
        ...
        {
            NotificationDetails notification = new NotificationDetails();
            MaterialMaster materialdetails = null;
            
            try
            {
                if (usernotifications.Code == (int)Enums.NotificationCode.CreateMRF || usernotifications.Code == (int)Enums.NotificationCode.CloseMRF)
                {
                    materialdetails = materialMasters.Where(m => m.MRF.ID == int.Parse(usernotifications.Reference)).FirstOrDefault();
                }
                else
                {
                    ...
                    materialdetails = materialStageAudit.MaterialMaster;
                }
                if (materialdetails != null)
                {
                    notification.ProjectID = materialdetails.ProjectID;
                    notification.NotificationID = usernotifications.ID;
                    notification.MaterialID = materialdetails.ID;
                    notification.Block = materialdetails.Block;
                    notification.Level = materialdetails.Level;
                    notification.Zone = materialdetails.Zone;
                    notification.MarkingNo = materialdetails.MarkingNo;
                    notification.MaterialType = materialdetails.MaterialType.Name;
                    if (usernotifications.Code == (int)Enums.NotificationCode.CreateMRF)
                        notification.Message = "A new MRF '" + materialdetails.MRF.MRFNo + "' has been created with '" + materialdetails.MRF.Materials.Count + "' materials";
                    else if (usernotifications.Code == (int)Enums.NotificationCode.CloseMRF)
                        notification.Message = "A new MRF '" + materialdetails.MRF.MRFNo + "' has been closed with '" + materialdetails.MRF.Materials.Count + "' materials";
                    else if (usernotifications.Code == (int)Enums.NotificationCode.MaterialQCFailed)
                        notification.Message = "QC for Material '" + materialdetails.Block + '-' + materialdetails.Level + '-' + materialdetails.Zone + '-' + materialdetails.MarkingNo + "' has been failed.";
                    else if (usernotifications.Code == (int)Enums.NotificationCode.MaterialQCPassed)
                        notification.Message = "QC for Material '" + materialdetails.Block + '-' + materialdetails.Level + '-' + materialdetails.Zone + '-' + materialdetails.MarkingNo + "' has been passed.";
                    notification.CreatedBy = materialdetails.MRF.CreatedBy.PersonName;
                    notification.IsMaterial = true;
                    ...
                    notification.Tag = materialdetails.Trackers.Select(t => t.Tag).FirstOrDefault();
