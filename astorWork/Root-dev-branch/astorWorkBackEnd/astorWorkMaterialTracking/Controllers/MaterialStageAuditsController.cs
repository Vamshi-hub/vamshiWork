...
namespace astorWorkMaterialTracking.Controllers
{
    ...
    public class MaterialStageAuditsController : CommonController
    {
        ...
        protected async Task<List<MaterialStage>> CreateMaterialStageCounts(int projectID)
        {
            ...
            try
            {
                materialStages = await _context.MaterialStageAudit
                                                    .Where(msa => msa.MaterialMaster.ProjectID == projectID)
                                                    .GroupBy(msa => msa.MaterialMasterID,
        ...
        public async Task<MaterialStageMaster> GetNextMaterialStageAudit([FromRoute] int material_id)
        {
            ...
            {
                materialMaster1 = await _context.MaterialMaster
                                                             .Include(mm => mm.MaterialType)
                                                             .Where(mm => mm.ID == material_id)
                                                             .FirstOrDefaultAsync();
                throw new GenericException(ErrorMessages.DbRecordNotFound, $"Material {materialMaster1.MaterialType.Name} not found.");
            }

            MaterialStageAudit currStage = materialMaster.StageAudits.OrderBy(sa => sa.CreatedDate).LastOrDefault();

            MaterialStageMaster nextStage = await GetNextStage(currStage, materialMaster);
