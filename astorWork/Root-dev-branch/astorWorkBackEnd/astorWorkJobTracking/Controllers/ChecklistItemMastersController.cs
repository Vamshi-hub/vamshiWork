    ...
    public class ChecklistItemMastersController : Controller
    {
        ...
        public async Task<SignedChecklist> ListChecklistItems([FromQuery] int checklist_id, [FromQuery] int job_schedule_id, [FromQuery] int material_id)
        {
            ...
            try
            {
                ...
                {
                    MaterialStageAudit materialStageAudit = _context.MaterialStageAudit.Where(ms => ms.MaterialMasterID == material_id).OrderByDescending(msa => msa.CreatedDate).FirstOrDefault();
            ...
            if (checklistAudits != null && checklistAudits.Count > 0)
            {
                ...
                List<ChecklistItemAudit> checklistItemAudits = await _context.ChecklistItemAudit
                                                                             ...
                                                                             .Where(cia => cia.ChecklistAuditID == checklistAudit.ID
                                                                                        && cia.ChecklistAudit.MaterialStageAudit.MaterialMasterID == material_id)
                                                                             .ToListAsync();
        ...
        public async Task<List<ChecklistItemUploadStatus>> ImportChecklistItems(IFormFile file, string project_id, [FromQuery] int trade_id, [FromQuery] string checklist_type, [FromQuery] int material_stage_id)
        {
            ...
            try
            {
                ...
                if (trade_id != 0)
                {
                    await InactiveTradeChecklistmaster(trade_id);
                    await InActiveTradeChecklistMasterAssociations(trade_id);
                }
                ...
                for (int i = 1; i < jobs.Count(); i++)
                {
                    ...
                    ChecklistMaster checklistMaster = await AddChecklistMaster(jobs.ElementAt(i), trade_id, material_stage_id);
                        ...
                        if (trade_id != 0)
                        {
                            ...
                            objChecklistMaster = _context.ChecklistMaster
                             .Where(cm => cm.TradeID == trade_id && cm.Name == checklistMaster.Name
                              && cm.Sequence == checklistMaster.Sequence)
                                                         .FirstOrDefault();
        ...
        private async Task InactiveTradeChecklistmaster(int trade_id)
        {
           try
            {
                List<ChecklistMaster> checklistmastertoDelete = await _context.ChecklistMaster.Where(cm => cm.TradeID == trade_id).ToListAsync();
        ...
        protected async Task InActiveTradeChecklistMasterAssociations(int trade_id)
        {
            try
            {
        ...
        private async Task<ChecklistMaster> AddChecklistMaster(string job, int tradeID, int materialStageID)
        {
            ChecklistMaster checklistMaster = CreateCheckListMaster(job, tradeID, materialStageID);
            try
            {
        ...
        protected ChecklistMaster CreateCheckListMaster(string checkListItemInfo, int? tradeID, int? MaterialStageID)
        {
        ...
        protected ChecklistItemMaster CreateChecklistItem(string checklistItemInfo, string checkListItemType)
        {
            ...
            {
                checklistItem.Type = (int)(ChecklistItemType)Enum.Parse(typeof(ChecklistItemType), checkListItemType);
            }
