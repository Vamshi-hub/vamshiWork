...
namespace astorWorkShared.Services
{
    public class AstorWorkForge : IAstorWorkForge
    {
        ...
        public async Task<DynamicDictionary> GetMetadata(string objectId)
        {
            ...
            ////((Autodesk.Forge.Model.DynamicDictionary)(new System.Collections.Generic.IDictionaryDebugView<string, object>(((Autodesk.Forge.Model.DynamicDictionary)result).Dictionary).Items[0]).Value).Dictionary.Values
            //DynamicDictionary objResult = (DynamicDictionary)result;
            //var r1 = objResult.Dictionary.Values;
            //var guid = new System.Collections.Generic.IDictionaryDebugView<string, object>(((Autodesk.Forge.Model.DynamicDictionary)(new System.Collections.Generic.IDictionaryDebugView<string, object>(((Autodesk.Forge.Model.DynamicDictionary)(new System.Collections.Generic.IDictionaryDebugView<string, object>(((Autodesk.Forge.Model.DynamicDictionary)(new System.Collections.Generic.IDictionaryDebugView<string, object>(((Autodesk.Forge.Model.DynamicDictionary)result).Dictionary).Items[0]).Value).Dictionary).Items[1]).Value).Dictionary).Items[0]).Value).Dictionary).Items[2].Value;
            //var v = _derivativesApi.GetModelviewPropertiesAsync(urn,guid.ToString());
            var data = result.ToJson()["data"]["metadata"][0].ToObject<ForgeMetadata>();
            return result;
        }
        public async Task<DynamicDictionary> GetMetadataProperties(string objectId, string viewGUID)
        {
            var urn = GetURN(objectId);
            var result = await _derivativesApi.GetModelviewPropertiesAsync(urn, viewGUID);
            return result;
        }
