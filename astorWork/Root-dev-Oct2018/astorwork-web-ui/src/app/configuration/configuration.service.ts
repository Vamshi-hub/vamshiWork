...
export class ConfigurationService {
  ...
  getVendors(): Observable<VendorMaster[]> {
  ...
  createVendor(vendor: VendorMaster) {
    const endpoint = `${environment.api_base_url}${this.url_base}/vendors`;
    return this.http.post(endpoint, vendor).pipe(
      catchError(this.handleError)
    );
  }

  updateVendor(vendorId: number, vendor: VendorMaster) {
    const endpoint = `${environment.api_base_url}${this.url_base}/vendors/${vendorId}`;
    return this.http.put(endpoint, vendor).pipe(
      catchError(this.handleError)
    );
  }

  getVendorDetails(vendorId: number): Observable<VendorMaster> {
    const endpoint = `${environment.api_base_url}${this.url_base}/vendors/${vendorId}`;
    return this.http.get<VendorMaster>(endpoint)
      .pipe(
        catchError(this.handleError)
  ...
  getmaterialsstages(): Observable<StageMaster[]> {
  ...
  updateSorting(lstStages: StageMaster[]) {
    ...
    return this.http.post(endpoint, lstStages).pipe(
      catchError(this.handleError)
    );
  }
