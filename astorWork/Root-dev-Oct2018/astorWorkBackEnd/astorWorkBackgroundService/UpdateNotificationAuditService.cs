...
namespace astorWorkBackgroundService
{
    public class UpdateNotificationAuditService
    {
        ...
        /// <returns></returns>
        public List<DeliveryMaterial> GetDelayedDeliveryMaterials(int projectOrVendorID, bool isProjectID)
        {
            IEnumerable<MaterialMaster> materialMasters = _context.MaterialMaster.Include(m => m.MRF)
                                                          ...
                                                          .Where
                                                          (
                                                            m => (isProjectID ? m.Project.ID == projectOrVendorID : m.Vendor.ID == projectOrVendorID)
                                                            &&
                                                            (
                                                            ...
                                                            (
                                                                m.StageAudits == null ||
                                                                m.StageAudits.OrderByDescending(s => s.Stage.Order).FirstOrDefault().Stage.Order < Convert.ToInt32(Enums.StageOrder.Delivered)
                                                            )
                                                          ).ToList();
            //&&
            //(
        ...
        /// <param name="projectID"></param>
        /// <returns></returns>
        public List<DeliveryMaterial> GetExpectedDeliveryMaterials(int projectOrVendorID, bool isProjectID)
        {
            IEnumerable<MaterialMaster> materialMasters = _context.MaterialMaster.Include(m => m.MRF)
                                                          .Include(m => m.Project)
                                                          .Include(v => v.Vendor)
                                                          ...
                                                          (
                                                            ...
                                                            && (m.StageAudits == null || m.StageAudits.Count <= 0 || m.StageAudits.OrderByDescending(s => s.Stage.Order).FirstOrDefault().Stage.Order < Convert.ToInt32(Enums.StageOrder.Delivered))
                                                          );
        ...
        protected DeliveryMaterial CreateDeliveryMaterial(MaterialMaster materialMaster)
        {
            ...
            delayedDeliveryMaterial.MaterialType = materialMaster.MaterialType;
        ...
        public void UpdateDeliveryNotificationAudit(astorWorkDbContext _astorWorkDbContext, int notificationTimerId, int notificationCode)
        {
            ...
            if (notificationCode == Convert.ToInt32(Enums.NotificationCode.DelayInDelivery))
                deliveryMaterials = GetDelayedDeliveryMaterials(projectOrVendorID, isProjectID);
            else
                deliveryMaterials = GetExpectedDeliveryMaterials(projectOrVendorID, isProjectID);

            List<UserMaster> receipients = AddNotificationReceipients(projectOrVendorID, deliveryMaterials, timer.Site != null ? timer.Site.ID : 0);

            if (receipients != null && receipients.Count > 0 && deliveryMaterials.Count > 0)
                UpdateNotificationAudit(receipients, Convert.ToInt32(notificationCode), Convert.ToInt32(Enums.NotificationType.Email), projectOrVendorID.ToString(), timer);
        }
