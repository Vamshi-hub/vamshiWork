    ...
    public class CommandPlugIn : CommandHandlerPlugin
    {
		...
		protected override void OnLoaded()
        {
            ...
            foreach (var tab in Autodesk.Windows.ComponentManager.Ribbon.Tabs)
            {
                ...
                {
                    if (Properties.Settings.Default.LAST_SYNC_TIME.Equals(DateTime.MinValue))
                        tab.Panels[0].Source.Title = "Never synced";
					else
                        tab.Panels[0].Source.Title = "Last synced at " + Properties.Settings.Default.LAST_SYNC_TIME.ToLongDateString();
                }
            }
        }
