...
namespace MaterialTrackApp.Utility
{
    public sealed class ApiClient
    {
        ...
        public async Task<ApiResult> MTGetMaterialMasterWithBeacons(IEnumerable<BeaconEntity> beacons)
        {
            var result = GENERIC_RESULT;
            try
            {
                JArray json = new JArray();
                string url = "Material/MasterWithBeacon?";

                foreach (var beacon in beacons)
                    url += "beaconIds=" + beacon.BeaconID + "&";
        ...
        public async Task<ApiResult> MTGetMaterialMasterPendingQC()
        {
            var result = GENERIC_RESULT;
            try
            {
                string url = "Material/MasterPendingQC";
        ...
        public async Task<ApiResult> MTGetMaterialForEnrolment(string vendorName)
        {
            var result = GENERIC_RESULT;
            try
            {
                string url = string.Format("Material/MaterialForEnrolment?vendorName={0}", vendorName);
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    Application.Current.Properties["max_lot_no"] = (int)json["MaxLotNo"];
        ...
        public async Task<ApiResult> MTGetMaterialForInstallation()
        {
            var result = GENERIC_RESULT;
            try
            {
                string url = "Material/MaterialForInstallation";
        ...
        public async Task<ApiResult> MTUpdateMaterialMasterBySite(List<SiteUpdateData> siteData)
        {
            var result = GENERIC_RESULT;
            try
            {
                string url = "Material/Master/UpdateBySite";
                string body = JsonConvert.SerializeObject(siteData);
                var response = await _client.PostAsync(url, new StringContent(body, Encoding.UTF8, "application/json"));
                if (response.IsSuccessStatusCode)
                {
                    var content = await response.Content.ReadAsStringAsync();
        ...
        public async Task<ApiResult> LoginUser(string userName, string password)
        {
            ...
            try
            {
                var url = "BeaconApp/Login";
                var data = new BeaconAppUser { UserName = userName, Password = password };
                string body = JsonConvert.SerializeObject(data);
                var response = await _client.PostAsync(url, new StringContent(body, Encoding.UTF8, "application/json"));
                if (response.IsSuccessStatusCode)
                {
                    var content = await response.Content.ReadAsStringAsync();
                    var user = JsonConvert.DeserializeObject<BeaconAppUser>(content);
                    var items = new List<BeaconAppUser> { user };
                    result = new ApiResult()
                    {
                        Status = 0,
                        Message = string.Empty,
        ...
        public partial class BeaconAppUser
        {
            ...
            public Nullable<int> LocationAssociationID { get; set; }
        }
    }
}
