        ...
        protected List<JobQCDetails> ListQCFailedMaterials(int project_id)
        {
            ...
            try
            {
                ...
                QCFailedMaterials = _context.ChecklistAudit
                             ...
                             .Where(j => j.JobScheduleID == null && j.MaterialStageAudit != null && j.MaterialStageAudit.Material.ProjectID == project_id
                        )
                .Select(list => new JobQCDetails
                {
                    Block = list.MaterialStageAudit.Material.Block,
                    Level = list.MaterialStageAudit.Material.Level,
                    Zone = list.MaterialStageAudit.Material.Zone,
                    CheckListName = list.Checklist.Name,
                    QcFailedBy = list.CreatedBy.PersonName,
                    QCStartDate = list.CreatedDate,
                    type = list.MaterialStageAudit.Material.MaterialType.Name,
                    MarkingNo = $"{list.MaterialStageAudit.Material.Block}-{list.MaterialStageAudit.Material.Level}-{list.MaterialStageAudit.Material.Zone}-{list.MaterialStageAudit.Material.MarkingNo}",
            ...
            try
            {
                ...
                else
                {
                    QCphotos = _context.ChatData
                                ...
                                .Include(c => c.ChecklistItem)
                                .Include(j => j.User)
                                .Where(j => j.ChecklistID == defect_id && j.MaterialStageAudit.Material.Project.ID == project_id && j.ThumbnailUrl != null && j.MaterialStageAuditID == MaterialStageAuditID)
                               .Select(list => new JobQCDetails
                               {
                                   ChecklistID = list.Checklist.ID,
                                   ID = list.ID,
