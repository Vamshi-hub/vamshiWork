...
namespace astorWorkJobTracking.Controllers
{
    ...
    public class MaterialTypeMastersController : Controller
    {
        ...
        // GET: api/<controller>
        [HttpGet]
        public async Task<List<MaterialType>> ListMaterialTypes([FromRoute] int project_id)
        {
            if (!ModelState.IsValid || project_id == 0)
                throw new GenericException(ErrorMessages.BadRequest, ErrorMessages.BadRequestMsg());

            var materialTypes = await _context.MaterialTypeMaster.Select(m => new MaterialType()
            {
                ID = m.ID,
                Name = m.Name,
                RouteTo = m.RouteTo
