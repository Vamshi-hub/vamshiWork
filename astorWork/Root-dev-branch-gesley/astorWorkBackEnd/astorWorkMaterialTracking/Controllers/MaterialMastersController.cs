...
namespace astorWorkMaterialTracking.Controllers
{
    ...
    public class MaterialMastersController : CommonController
    {
        ...
        public async Task<List<MaterialDetail>> ListMaterials([FromRoute] int project_id, [FromQuery] string block)
        {
            ...
            if ((user.RoleID == (int)Enums.RoleType.VendorProjectManager || user.RoleID == (int)Enums.RoleType.VendorProductionOfficer) && user.Organisation != null)
        ...
        [HttpGet("{id}")]
        public async Task<MaterialDetail> GetMaterial([FromRoute] int project_id, [FromRoute] int id)
        {
            if (!ModelState.IsValid)
                throw new GenericException(ErrorMessages.BadRequest, ErrorMessages.BadRequestMsg());

            // Retrieve material detail based on ID
            MaterialDetail material = await CreateMaterialDetail(id);
                ...
                if (materials.Count > 0)
                {
                    try
                    {
                        ...
                        if (await _context.MaterialStageMaster.AnyAsync(msm => msm.MaterialTypes.Contains(importTemplate.MaterialType)))
                            _context.MaterialStageMaster.UpdateRange(await AddNewMaterialToAllStages(importTemplate.MaterialType));
        ...
        public async Task<List<TrackerAssociation>> GetMainConProducedMaterials([FromRoute]int project_id, [FromQuery] string tenant_name)
        {
            ...
            {
                UserMaster user = await _context.GetUserFromHttpContext(HttpContext);
                List<MaterialMaster> producedMaterials = await _context.MaterialMaster
        ...
        protected TrackingHistory CreateStageTrackingHistory(MaterialStageAudit materialStageAudit)
        {
            return new TrackingHistory()
            {
                ...
                IsQCStage = false,
                StageID=materialStageAudit.Stage.ID
            };
        }
        ...
        protected TrackingHistory CreateCaseTrackingHistory(MaterialMaster materialMaster, IEnumerable<MaterialQCCase> materialQCCases, List<ChecklistAudit> checklists, TrackingHistory materialstageaudit)
        {
            ...
            if (checklists != null && checklists.Count > 0)
            {
                ...
                var chkPassedList= checklists.Where(s => s.Status == Enums.QCStatus.QC_passed_by_Maincon || s.Status == Enums.QCStatus.QC_accepted_by_RTO).GroupBy(ch => new { ch.ChecklistID, ch.MaterialStageAuditID, ch.JobScheduleID }).Select(c => new JobQCDetails
                {
                    ChecklistID = c.Key.ChecklistID,
                    ID = Convert.ToInt32(c.Key.MaterialStageAuditID),
                    TradeID = c.Key.JobScheduleID != null ? Convert.ToInt32(c.Key.JobScheduleID) : 0,
                ...
                trackingHistory.StageStatus = trackingHistory.StageStatus == 2 ? 2
            : materialMaster.StageAudits.Where(m => m.ID == materialstageaudit.ID).FirstOrDefault().QCStatus == Enums.JobStatus.All_QC_passed || materialMaster.StageAudits.Where(m => m.ID == materialstageaudit.ID).FirstOrDefault().QCStatus == Enums.JobStatus.QC_passed_by_Maincon ?1
                   ...
                   || materialMaster.StageAudits.Where(m => m.ID == materialstageaudit.ID).FirstOrDefault().QCStatus == Enums.JobStatus.QC_rejected_by_RTO 
                   ? 2 : materialMaster.StageAudits.Where(m => m.ID == materialstageaudit.ID).FirstOrDefault().QCStatus == Enums.JobStatus.Job_not_assigned ? 1 : 3;
                ...
                trackingHistory.TotalStructPassCount = chkPassedList.Where(c => c.ID == materialstageaudit.ID && c.TradeID == 0).Count();
                trackingHistory.TotalArchiChecklistCount = chk.Where(c => c.ID == materialstageaudit.ID && c.TradeID > 0).Count();
                trackingHistory.TotalArchiPassCount = chkPassedList.Where(c => c.ID == materialstageaudit.ID && c.TradeID > 0).Count();
        ...
        protected List<TrackingHistory> AddCaseTrackingHistories(MaterialMaster materialMaster, List<TrackingHistory> trackingHistories, List<ChecklistAudit> checklistAudits,List<ChecklistMaster> totalchecklists)
        {
        ...
        protected List<TrackingHistory> CreateMaterialTrackingHistory(MaterialMaster materialMaster, List<ChecklistAudit> checklistAudits,List<ChecklistMaster> totalchecklists)
        {
            ...
            trackingHistories = AddCaseTrackingHistories(materialMaster, trackingHistories, checklistAudits, totalchecklists);

            return AddSubsequentStages(materialMaster, trackingHistories);
        }

        protected async Task<MaterialDetail> CreateMaterialDetail(int materialID)
        {
            ...
            List<MaterialStageAudit> materialStageAudits = materialMaster.StageAudits;

            List<ChecklistAudit> checklistAudits = await _context.ChecklistAudit
                                                                 .Include(chk => chk.MaterialStageAudit)
                                                                 .Where(chk => materialStageAudits != null && materialStageAudits.Count > 0 && chk.MaterialStageAudit != null && materialStageAudits.Contains(chk.MaterialStageAudit))
                                                                 .ToListAsync();
            List<ChecklistMaster> totalchecklists = await _context.ChecklistMaster.ToListAsync();
            ...
            materialDetail.TrackingHistory = CreateMaterialTrackingHistory(materialMaster, checklistAudits, totalchecklists);
        ...
        protected MaterialDetail CreateMaterialInfo(int materialID, MaterialMaster materialMaster)
        {
            ...
            MaterialDrawingAssociation drawingAssociation = materialMaster.DrawingAssociations
                                                                          .OrderByDescending(a => a.Drawing.DrawingIssueDate)
                                                                          .FirstOrDefault();
            if (drawingAssociation != null)
                materialDetail.DrawingNo = drawingAssociation.Drawing.DrawingNo;
        ...
        protected async Task<List<MaterialStageMaster>> AddNewMaterialToAllStages(string materialType)
        {
        ...
        protected async Task<List<MaterialDetail>> CreateMaterialList(List<MaterialMaster> materialMasters)
        {
            MaterialStageMaster deliveredStage = await GetDeliveredStage();


            return materialMasters.Select(m => new MaterialDetail
            {
                ID = m.ID,
                Block = m.Block,
                Level = m.Level,
                ...
                TrackerTag = GetQRTracker(m.Trackers)?.Tag,
                TrackerLabel = GetQRTracker(m.Trackers)?.Label,
                TrackerType = GetQRTracker(m.Trackers)?.Type,
                MaterialType = m.MaterialType.Name,
                OpenQCCaseID = GetOpenQCCaseID(m.QCCases),
                MRFNo = m.MRF == null ? null : m.MRF.MRFNo,
                ExpectedDeliveryDate = GetExpectedDeliveryDate(m.MaterialInfoAudits, m.MRF),
                StageOrder = GetLatestStageAudit(m.StageAudits) == null ? 0 : GetLatestStageAudit(m.StageAudits).Stage.Order,
                StageName = GetLatestStageAudit(m.StageAudits) == null ? null : GetLatestStageAudit(m.StageAudits).Stage.Name,
                StageColour = GetLatestStageAudit(m.StageAudits) == null ? null : GetLatestStageAudit(m.StageAudits).Stage.Colour,
                ...
                QCStatusCode = GetQCStatusCode(m.QCCases, m.StageAudits),
                QCStatus = GetQCStatus(m.QCCases, m.StageAudits)
            }).ToList();
        }
            ...
            try
            {
                ...
                List<int> materialIDs = await _context.ChecklistAudit.Include(r => r.RouteTo)
                                                                     ...
                                                                     .Where(ca => ca.RouteTo == rto && ca.Checklist.MaterialStage != null
                                                                                                    && ca.MaterialStageAudit.Material.ProjectID == projectID)
                                                                     .Select(ca => ca.MaterialStageAudit.Material.ID)
                                                                     .ToListAsync();
        ...
        protected List<TrackerAssociation> GetInventoryList(IEnumerable<MaterialMaster> materials, string tenant_name)
        {
            ...
            try
            {
                trackerAssociations = materials.Select(mm => new TrackerAssociation
                {
                    Trackers = mm.Trackers.Select(t => CreateTracker(t)).ToList(),
                    Material = new MaterialMobile
                    {
                        ...
                        IsQCOpen = mm.QCCases.Where(qc => qc.Defects.Any(d => d.Status == Enums.QCStatus.QC_failed_by_Maincon)).Count() > 0
                    }
                }).ToList();
            }
            catch (Exception ex)
        ...
        {
            return await _context.MaterialStageMaster.FirstOrDefaultAsync(mm => mm.MilestoneID == (int)Enums.MileStoneID.Delivered);
        }

        protected int GetOpenQCCaseID(List<MaterialQCCase> qcCases)
        {
            bool noOpenQCCase = qcCases == null || qcCases.Count == 0 || qcCases.Where(d => d.Defects.Any(q => q.Status != Enums.QCStatus.QC_passed_by_Maincon)).Count() == 0;
            return noOpenQCCase ? 0 : qcCases.Where(d => d.Defects.Any(q => q.Status != Enums.QCStatus.QC_passed_by_Maincon)).FirstOrDefault().ID;
        }

        protected DateTimeOffset? GetExpectedDeliveryDate(List<MaterialInfoAudit> materialInfoAudits, MRFMaster mrf)
        {
            MaterialInfoAudit latestMaterialInfoAudit = materialInfoAudits.OrderByDescending(mi => mi.CreatedDate).FirstOrDefault();
            return latestMaterialInfoAudit == null ?
                   (mrf == null ? null : mrf?.ExpectedDeliveryDate) :
                   latestMaterialInfoAudit.ExpectedDeliveryDate;
        }

        protected MaterialStageAudit GetLatestStageAudit(List<MaterialStageAudit> stageAudits)
        {
            return stageAudits.OrderByDescending(s => s.CreatedDate).FirstOrDefault();
        }

        protected int GetQCStatusCode(List<MaterialQCCase> qcCases, List<MaterialStageAudit> stageAudits)
        {
            if (qcCases == null || qcCases.Count == 0 && stageAudits == null ||stageAudits.Count==0)
                return 0;

            MaterialStageAudit latestMaterialStageAudit = stageAudits.OrderByDescending(s => s.CreatedDate).FirstOrDefault();
            Enums.JobStatus latestMaterialStageAuditQCStatus = latestMaterialStageAudit.QCStatus;

            return (HasQCCasesWithOpenDefects(qcCases) ? (int)Enums.QCStatus.QC_failed_by_Maincon :
                    NoStageChecklists(stageAudits, latestMaterialStageAudit) ? -1 :
                    latestMaterialStageAuditQCStatus == Enums.JobStatus.Job_not_assigned ? (int)Enums.QCStatus.Pending_QC :
                    latestMaterialStageAuditQCStatus == Enums.JobStatus.All_QC_passed ? (int)Enums.QCStatus.QC_rectified_by_Subcon :
                    latestMaterialStageAuditQCStatus == Enums.JobStatus.QC_failed_by_Maincon || latestMaterialStageAuditQCStatus == Enums.JobStatus.QC_rejected_by_RTO ? (int)Enums.QCStatus.QC_failed_by_Maincon :
                                                                                                                                                                         (int)Enums.QCStatus.QC_passed_by_Maincon);
        }

        protected bool HasQCCasesWithOpenDefects(List<MaterialQCCase> qcCases)
        {
            return qcCases != null && qcCases.Count > 0 && qcCases.Where(d => d.Defects.Any(q => q.Status != Enums.QCStatus.QC_passed_by_Maincon)).Count() > 0;
        }

        protected bool NoStageChecklists(List<MaterialStageAudit> stageAudits, MaterialStageAudit latestMaterialStageAudit)
        {
            return stageAudits == null
                || stageAudits.Count == 0
                || latestMaterialStageAudit.Stage.checklists == null
                || latestMaterialStageAudit.Stage.checklists.Count == 0;
        }

        protected string GetQCStatus(List<MaterialQCCase> qcCases, List<MaterialStageAudit> stageAudits)
        {
            if (qcCases == null || qcCases.Count == 0&&stageAudits==null || stageAudits.Count == 0)
                return "NA";
            ...
            return HasQCCasesWithOpenDefects(qcCases) ? "Failed" :
                   NoStageChecklists(stageAudits, latestMaterialStageAudit) ? "NA" :
                   latestMaterialStageAudit.QCStatus == Enums.JobStatus.Job_not_assigned ? "Qc Pending" :
                   latestMaterialStageAudit.QCStatus.ToString().Replace('_', ' ');
        }

        protected TrackerMaster GetQRTracker(List<TrackerMaster> trackers)
        {
            string tagTypeQRCode = Enums.TagType.QR_Code.ToString().Replace('_', ' ');
            return trackers?.Where(t => t.Type == tagTypeQRCode).FirstOrDefault();
        }
    }
}