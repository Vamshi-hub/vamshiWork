...
namespace astorWorkMaterialTracking.Controllers
{
    ...
    public class MaterialQCController : CommonQCController
    {
        ...
        public async Task<List<QCCase>> ListQCCases([FromQuery] int? project_id, [FromQuery] int? material_id)
        {
            ...
            List<MaterialQCCase> materialQCCases = await _context.MaterialQCCase
            ...
            if (material_id.HasValue)
                materialQCCases = materialQCCases.Where(qc => qc.MaterialMasterId == material_id.Value).ToList();
            else if(project_id.HasValue)
                materialQCCases = materialQCCases.Where(qc => qc.MaterialMaster?.ProjectID == project_id.Value).ToList();
            ...
            foreach (MaterialQCCase materialQCCase in materialQCCases)
                qcCases.Add(CreateQCCase(materialQCCase));
