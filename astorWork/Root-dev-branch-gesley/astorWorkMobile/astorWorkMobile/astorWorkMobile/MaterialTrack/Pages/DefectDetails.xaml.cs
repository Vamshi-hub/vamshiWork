...
namespace astorWorkMobile.MaterialTrack.Pages
{
    ...
    public partial class DefectDetails : ContentPage
    {
        ...
        private async void AddPhotoClicked(object sender, EventArgs e)
        {
            var action = await Application.Current.MainPage.DisplayActionSheet("Attach a photo", "Cancel", null, "Gallery", "Camera");
            if (action == "Gallery")
            {
                Device.BeginInvokeOnMainThread(async () =>
                {
                    var result = await ViewModelLocator.mtTakePhotoVM.GetPhotoFromGallery();
                    if (result)
                        await ConfirmPhoto();
                });
            ...
            else if (action == "Camera")
            {
                Device.BeginInvokeOnMainThread(async () =>
                {
                    var result = await ViewModelLocator.mtTakePhotoVM.CapturePhoto();
                    if (result)
                        await ConfirmPhoto();
                });
            }
