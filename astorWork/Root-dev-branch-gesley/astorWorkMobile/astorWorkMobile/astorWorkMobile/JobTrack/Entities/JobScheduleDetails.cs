...
namespace astorWorkMobile.JobTrack.Entities
{
    public class JobScheduleDetails : MasterVM
    {
        ...
        public bool ShowChecklistButton
        {
            ...
            {
                bool status = false;
                if (Convert.ToInt32(Application.Current.Properties["entry_point"]) == 1 && RouteToRTO)
                {
                    if (RtoInspectionStatusCode == Convert.ToInt32(InspectionStage.Ongoing) && StatusCode >= JobStatus.Job_started && StatusCode < JobStatus.All_QC_passed)
                    {
                        status = true;
                    }
                    else if (RtoInspectionStatusCode == Convert.ToInt32(InspectionStage.After) && StatusCode >= JobStatus.Job_completed && StatusCode < JobStatus.All_QC_passed)
                    {
                        status = true;
                    }
                    else
                ...
                else if (Convert.ToInt32(Application.Current.Properties["entry_point"]) == 3)
                {
        ...
        private async void CheckListClicked()
        {
            ...
            if (Convert.ToInt32(Application.Current.Properties["entry_point"]) == 1)
            {
                ////await ApiClient.Instance.INGetInspectionForm(ID).ContinueWith(async t =>
                //{
                //    if (t.Result.status == 0)
                //    {
                //        IsLoading = false;
                ViewModelLocator.rtoInspectionVM.JobScheduleID = ID;
                await Navigation.PushAsync(new RTOInspectionForm());
                //    }
                //    else
                //    {
                //        ErrorMessage = t.Result.message;
