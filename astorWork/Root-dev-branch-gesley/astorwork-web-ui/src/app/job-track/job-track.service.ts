...
export class JobTrackService {
  ...
  getScheduleJobs(projectID: number,lastMaterialIndex:number,pageSize:number,blk: string, lvl: string, materialType: string, subcon: string,job_Status:number) {
    const endpoint = `${environment.api_base_url}${this.url_base}/projects/${projectID}/job-schedule/jobschedulelist?lastMaterialIndex=${lastMaterialIndex}&pageSize=${pageSize}&block=${blk}&level=${lvl}&materialType=${materialType}&subcon=${subcon}&job_Status=${job_Status}`;
    return this.http.get<JobSchedule[]>(endpoint, this.common_http_options)
      .pipe(
        catchError(this.handleError)
      );
