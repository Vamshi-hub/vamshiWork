    ...
    this.route.parent.data.subscribe(async (data: { project: ProjectMaster }) => {
      ...
      if (project) {
        ...
        await this.route.paramMap.subscribe((params: Params) => {
          ...
          this.filterValue =params.get('module') == null ? '': params.get('module');
          if ((this.jobid != null) || this.filterValue || this.filterstatus)
      ...
      data => {
        ...
        for(var i=0;i<=data.length-1;i++) {
          ...
          if(this.filterValue){
            ...
            this.caseDataSource = new MatTableDataSource(this.listCases.filter(s=>s.markingNo==this.filterValue));
            console.log( this.caseDataSource);
          }
          else if(this.filterstatus)
          {
            console.log(this.filterstatus);
            this.caseDataSource = new MatTableDataSource(this.listCases.filter(s=>s.status==this.filterstatus));
            console.log( this.caseDataSource);
          }
