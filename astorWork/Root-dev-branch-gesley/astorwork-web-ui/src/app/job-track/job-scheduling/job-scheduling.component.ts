...
export class JobSchedulingComponent extends CommonLoadingComponent {
  ...
  filterJOBS = '';
  jobid= '';
  JobstatusDummyCode=-1; // For Mobile Purpose same api using
  subconsDataSource: MatTableDataSource<OrganisationMaster> = new MatTableDataSource();
  displayedColumns = ['level', 'materialType', 'zone', 'markingNo', 'tradeName', 'subcon', 'plannedStartDate', 'plannedEndDate', 'status'];
  ...
  //  this.jobTrackService.getJobs(this.project.id, this.block, this.level, this.materialType, this.subcon, lastMaterialIndex).subscribe(
    this.jobTrackService.getScheduleJobs(this.project.id,this.lastMaterialIndex,this.pageSize,this.block, this.level, this.materialType, this.subcon,this.JobstatusDummyCode).subscribe(
      data => {     
        console.log(data);
