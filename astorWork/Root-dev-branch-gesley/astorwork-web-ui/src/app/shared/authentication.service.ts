...
import { TenantSettings } from './classes/tenant_settings';
...
export class AuthenticationService {
  ...
  tenantSettings():Observable<TenantSettings> {
    const endpoint = `${environment.api_base_url}/tenant/settings`;
    return this.http.get<TenantSettings>(endpoint).pipe(
      catchError(this.handleError)
    );
  }
  ...
  async logOut() {
    ...
    window.localStorage.removeItem('tenant_settings');
    window.sessionStorage.clear();
    this.router.navigateByUrl('/');
    var tenantsettings = await this.tenantSettings().toPromise();
      window.localStorage.setItem('tenant_settings', JSON.stringify(tenantsettings));
  }
