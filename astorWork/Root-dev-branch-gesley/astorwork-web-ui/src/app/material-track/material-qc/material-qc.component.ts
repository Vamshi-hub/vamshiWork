    ...
    this.route.parent.data.subscribe(async (data: { project: ProjectMaster }) => {
      ...
      if (project) {
        ...
        await this.route.paramMap.subscribe((params: Params) => {
          ...
          this.filterstatus = params.get('filter');
          this.filterValue = params.get('module') == null ? '' : params.get('module') + " " + params.get('stage');
          if ((this.jobid != null) ||this.filterstatus || this.filterValue )
  ...
  getfilterdata() {
    this.QCFailedstrctDatasource.filterPredicate = (data: Jobstarted, filter: string) => {
      ...
      for (let entry of filterArray) {
        ...
        if (data.status.toLowerCase().indexOf(entry) >= 0 ||
        data.qcFailedBy.toLowerCase().indexOf(entry) >= 0 ||
        data.markingNo.toLowerCase().indexOf(entry) >=0 || data.stageName.toLowerCase().indexOf(entry) >=0)
          countMatch++;
    ...
    this.jobtrackservice.getStructuraljobs(projectID).subscribe(
      data => {
        ...
        for(var i=0;i<=data.length-1;i++) {
          ...
          {
            this.QCFailedstrctDatasource = new MatTableDataSource(this.liststrct.filter(s=>s.status==this.filterstatus));
          }
