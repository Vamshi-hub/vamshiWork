...
export class CreateMrfComponent implements OnInit {
  ...
  submitted = false;
  blocks: string[];
  isallzoneSelected = false;
  isalllevelSelected = false;
  isallmaterialtypesSelected = false;
...
selected=false;
  ...
  levelarray:string[];
  zones: string[];
  @ViewChild('mylevelSel') mylevelSel: MatSelect;
  @ViewChild('myzoneSel') myzoneSel: MatSelect;
  @ViewChild('mymaterialtypeSel') mymaterialtypeSel: MatSelect;
  @ViewChild('alllevelSelected') private alllevelSelected: MatOption;
  @ViewChild('allzoneSelected') private allzoneSelected: MatOption;
  @ViewChild('allmaterialtypeSelected') private allmaterialtypeSelected: MatOption;
  /*
    mrf = {
      orderDate: moment(),
      block: '',
  ...
  */
  ...
  setDefaultVendorSelection(){
   var levelsselect;
  var selectzone;
    if(this.isalllevelSelected)
    { 
     levelsselect= this.theForm.controls.level.value.filter((x, i) => i !== 0);
    }
    else{
      levelsselect=this.theForm.controls.level.value
    }
    if(this.allzoneSelected){
       selectzone=this.theForm.controls.zone.value.filter(y=>y!==0);
    }
    
    if(levelsselect.length>0 &&selectzone.length>0){
    this.materialTrackService.getMaterialTypesForNewMRF(
      this.project.id,
      this.theForm.controls.block.value,
      levelsselect,
      selectzone).subscribe(
        data => {
        ...
        error => {
          console.log(this.alllevelSelected,'all')
console.log(this.isalllevelSelected,'selection');
          this.uiUtilService.closeAllDialog();
    ...
    if (!opened && this.theForm.controls.block.value != null) {
      ...
      this.materialTrackService.getLocationForNewMRF(this.project.id, this.theForm.controls.block.value).subscribe(
        data => {
          ...
          this.levelarray=Array.from(this.levels);
  ...
  zoneOpened(opened: boolean) {  
    ...
    if (!opened && str!==null && str.length>0 && str!=='') {
      // this.uiUtilService.openDialog(SpinnerDlgComponent, null, true);
      this.setDefaultSelection();
    }
  ...
  */
  ...
  ZoneAllSelection() {
      this.isallzoneSelected = !this.isallzoneSelected;  // to control select-unselect
      
      if (this.isallzoneSelected) {
        this.myzoneSel.options.forEach( (item : MatOption) => item.select());

      } else {
        this.myzoneSel.options.forEach( (item : MatOption) => {item.deselect()});
      }
  }
  togglezone(all){ 
    if (this.allzoneSelected.selected) {  
     this.allzoneSelected.deselect();
     this.isallzoneSelected=false;
     return false;
 }
   if(this.theForm.controls.zone.value.length==this.zones.length)
     this.allzoneSelected.select();
 
 }
  LevelAllSelection() {
    this.isalllevelSelected = !this.isalllevelSelected; 
    if (this.isalllevelSelected) {
      this.mylevelSel.options.forEach( (item : MatOption) => {item.select()}
      );
    } else {
      this.mylevelSel.options.forEach( (item : MatOption) => {item.deselect()});
    }
  } 
  togglelevel(all){ 
    if (this.alllevelSelected.selected) {  
     this.alllevelSelected.deselect();
     this.isalllevelSelected=false;
     return false;
 }
   if(this.theForm.controls.level.value.length==this.levelarray.length)
     this.alllevelSelected.select();
 
 } 
  MaterialtypesAllSelection() {
    this.isallmaterialtypesSelected = !this.isallmaterialtypesSelected; 
    if (this.isallmaterialtypesSelected) {
      this.mymaterialtypeSel.options.forEach( (item : MatOption) => item.select());
    } else {
      this.mymaterialtypeSel.options.forEach( (item : MatOption) => {item.deselect()});
    }
  }
  togglematerialtype(all){ 
    if (this.allmaterialtypeSelected.selected) {  
     this.allmaterialtypeSelected.deselect();
     return false;
 }
   if(this.theForm.controls.materialTypes.value.length==this.materialTypes.length)
     this.allmaterialtypeSelected.select();
