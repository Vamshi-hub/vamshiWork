...
import { SampleViewFileComponent } from './sample-view/sample-view.component';
...
export class ImportFileComponent extends CommonLoadingComponent {
  ...
  sampleTemplate()
  {
    this.uiUtilService.openDialog(SampleViewFileComponent, true);
  }
  ...
  onFileChange(event) {
    ...
    if (event.target.files && event.target.files.length) {
      ...
      reader.onload = (e) => {
        ...
        if (lines.length > 0 && lines[0].length > 1 && lines[0][0] == 'Level' && lines[0][1] == 'Zone' && lines[0][2] == 'MarkingNo') {
