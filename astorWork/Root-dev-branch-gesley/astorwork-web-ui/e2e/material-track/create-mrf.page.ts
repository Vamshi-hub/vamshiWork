...
export class CreateMRFPage {
    ...
    fillForm(blkIndex: number, level: string, zone: string, organisationIndex: number) {
        ...
        // Select organisation
        var organisationSelect = element(by.css('mat-select[placeholder="Organisation"]'));
        browser.wait(EC.visibilityOf(organisationSelect), 500, "Organisation select not visible").then(() => {
            var activeOptions = element.all(by.className('option-organisation'));
            browser.actions().mouseMove(organisationSelect).click().perform().then(function () {
                browser.wait(EC.visibilityOf(activeOptions.get(0)), 1000, 'Organisation option is not visible').then(() => {
                    browser.actions().mouseMove(activeOptions.get(organisationIndex)).click().perform();
                })
            });
        });
